...
기획서의 `예시`, `약`, `00`, `미정`이나 단일 TEST 입출력은 실제 금액·비율·성과·정책으로
일반화하지 않습니다. 사용자에게 보이는 핵심 사실이 확실하지 않으면 현재 근거, 가장 적합한 권장안,
대안별 영향을 한 번에 보여주고 값 하나를 확인합니다. 답과 무관한 구현·검증은 계속하지만, 관련
계산·공개 문구·외부 실행·production 배포는 확인될 때까지 차단합니다. `TEST-01`, `임시 가정`,
`운영자 확인 후 변경` 같은 내부 불확실성 문구를 사용자 화면에 대신 노출하지 않습니다.
...
구현 중 기본 기록 파일은 원본 `SOURCE_PLAN.md`와 한 화면짜리 검증 상태판 `PLAN.md` 두 개뿐입니다.
`PLAN.md`는 계약 생성, 중요한 사람 결정, 실제 blocker handoff, 최종 판정에서만 갱신합니다. 작은
수정·설치·테스트·배포마다 문서를 고치거나 내부 단계 완료를 설명하지 않습니다. 모든 완료 게이트를
통과하면 `MEMORY.md`를 생성해 최종 검증 결과를 marker 블록 하나로 기록합니다. 재검증에서는 같은
블록을 갱신해 중복 기록을 만들지 않습니다. `AGENTS.md`·`CLAUDE.md`·`CASE.md`는 프로젝트 고유 인계나
교육 사례를 사용자가 요청했을 때만 만듭니다. 기록은 줄여도 결과물별 테스트와 실제 동작 검증은
그대로 실행합니다.
...
- `SOURCE_PLAN.md` — App에서 가져오거나 인터뷰로 만든 원본 기획서. 기본 생성
- `PLAN.md` — REQ·TEST·최종 증거를 중복 없이 한 표에 연결한 실행 계약. 기본 생성
- `DESIGN.md` — 사용자가 `orange-design`에서 승인한 디자인 원칙이 있을 때만 생성
- `MEMORY.md` — 구현 완료 시 생성하며 최종 검증 결과를 정확히 한 번 기록
- `CASE.md` — 사용자가 교육 사례 카드를 요청했을 때만 생성

완료는 필수 REQ와 TEST가 모두 PASS이고 실제 증거가 있을 때만 선언합니다. 일부가 미검증이면
`N/M 통과`와 남은 항목을 그대로 보여줍니다. App에서 시작한 작업은 사용자가 원할 때 GitHub URL,
배포 URL과 완료 시 만든 `MEMORY.md`를 App에 되돌릴 수 있으며, 파일 업로드와 참가자 회고 작성은 사용자의 명시적
동의를 받은 뒤에만 보조합니다.
