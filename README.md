...
기본 기록 파일은 원본 `SOURCE_PLAN.md`와 한 화면짜리 검증 상태판 `PLAN.md` 두 개뿐입니다.
`PLAN.md`는 계약 생성, 중요한 사람 결정, 실제 blocker handoff, 최종 판정에서만 갱신합니다. 작은
수정·설치·테스트·배포마다 문서를 고치거나 내부 단계 완료를 설명하지 않습니다. `AGENTS.md`·
`CLAUDE.md`·`MEMORY.md`·`CASE.md`는 자동 생성하지 않으며, 프로젝트 고유 인계나 과정 기록을 사용자가
요청했을 때만 필요한 파일 하나를 만듭니다. 기록은 줄여도 결과물별 테스트와 실제 동작 검증은 그대로
실행합니다.
...
- `MEMORY.md` — 사용자가 과정 기록을 요청했을 때만 생성
...
배포 URL과 요청해 만든 `MEMORY.md`를 App에 되돌릴 수 있으며, 파일 업로드와 참가자 회고 작성은 사용자의 명시적
동의를 받은 뒤에만 보조합니다.
