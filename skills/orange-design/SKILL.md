...
- 채택한 모든 시각 수정마다 `MEMORY.md`를 바꾸지 않는다. 새 디자인 시스템·외부 참고자료·브랜드
  방향처럼 다음 작업의 판단 기준이 되는 결정을 채택했거나 중요한 회귀를 해결했더라도, 사용자가
  과정 기록 유지를 요청한 경우에만 디자인 근거, 고정한 기능, 변경 변수, 검증 결과를 한 항목으로
  남긴다.
