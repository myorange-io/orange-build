...
선택한 값과 한 줄 이유는 `PLAN.md` 메타데이터에 남긴다. 기존 경로를 재사용하거나 정상적으로
`codex_sites`를 선택한 것만으로 `MEMORY.md`를 수정하지 않는다. 배포 공급자 폴백이나 비용·계정·
공개 범위를 바꾸는 사람 결정이 생긴 경우에만 그 이유를 마일스톤 기록으로 남긴다. 폴백은 원본
기능을 줄이는 근거가 아니다. 폴백에 새 가입·비용·계정 선택이 필요할 때만 기존 사람 게이트를 적용한다.
