...
- 전체 업무의 인지 과업 지도에서 사람이 맡을 일·AI 후보·이번 스킬 과업이 구분됨
- `skill_scope: atomic`, `atomicity: CONFIRMED`, 입력→한 가지 처리→주 출력 한 개의 책임 확인
- 하지 않을 일·사람 판단·다음 스킬 handoff가 분리되고 다른 미검증 스킬 없이 독립 가치가 있음
...
- 내장 평가의 구조·대표 호출·긍정·비트리거·빈/깨진 입력·도구 부재·새 작업·사람 검토가
  `PASS | FAIL | EVIDENCE_MISSING`로 판정되고 숫자 점수를 쓰지 않음
...
기본은 결과물 안의 계약·fixture·실제 호출과 사람이 확인하는 내장 평가다. 별도 심판이나 reviewer를
항상 추가하지 않는다. 다음 중 하나가 있을 때만 구현과 분리된 사람 또는 모델에게 `SOURCE_PLAN.md`,
`PLAN.md`, 구현과 증거를 주고 독립 검토를 활성화한다.

- 같은 경계에서 두 번 이상 회귀하거나 자동 수정이 반복됨
- 권한·개인정보·실데이터·외부 발송처럼 실패 비용이 큼
- 테스트와 실제 관찰이 상충하거나 증거 출처가 한쪽뿐임
- 표현·품질 기준이 모호해 계약만으로 PASS/FAIL이 갈림
- 두 구현·버전 중 하나를 비교 선택해야 함

활성화 조건이 없으면 `independent_review: NOT_NEEDED`와 이유를 남기고 아래 체크리스트를 최종
self-review에 한 번만 사용한다. 활성화했다면 과정 설명을 문서에 복제하지 않고 결함과 근거를
`PASS | FAIL | EVIDENCE_MISSING`로 남긴다. 숫자 점수나 심판의 심판을 만들지 않는다.
