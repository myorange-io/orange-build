...
목표: 구현 전에 필요한 도구·계정·권한·브라우저 설정을 조용히 확인하고, 실제 변경이나 사용자
동작이 필요한 항목만 보여준다. `helpful-tools.md`로 부족한 개발 능력을 판정하되 준비된 항목과 제외한
...
## 2. 변경이 필요한 경우만 준비 카드 보여주기

설치나 브라우저 이동 전에 상태를 읽기 전용으로 확인한다. 프로필과 관계없이 실제 변경이나 사용자
동작이 필요할 때만 아래 형식으로 한 번 알린다.

```text
변경분 준비 카드
- 설치 승인: [없으면 생략 / 도구·scope·영향]
- 사용자 동작: [없으면 생략 / 가입·2FA·계정 선택 한 동작]
- 권한·비용: [없으면 생략 / 승인할 정확한 영향]
```

준비된 도구, 선택하지 않은 서비스, 내부 판정 근거는 대화에 나열하지 않는다. `PLAN.md`의
`## 사전 준비`에는 미해결 준비물, 설치 동의, 계정·권한 선택처럼 재개에 필요한 항목만 한 줄씩
기록한다. 전부 준비됐다면 `없음` 한 줄을 유지하고 검사 결과를 복제하지 않는다. `REQUIRED` 준비물은
...
### `USER_ACTION`을 요청하는 형식

자동 설치·브라우저 대행·읽기 전용 확인으로 진행할 수 있는 동안에는 이 안내를 띄우지 않는다.
비밀번호·2FA·본인확인·계정 선택처럼 사용자만 할 수 있는 한 동작에서 실제로 멈췄을 때만 다음
형식을 쓴다.

```text
지금 할 한 동작: [현재 화면에서 사용자가 할 일 하나]
완료 신호: [무엇이 보이면 끝인지]
제가 이어서 할 일: [완료 직후 에이전트가 할 다음 작업]
남은 단계: [최대 8개의 짧은 단계 또는 N개]
```

- 한 안내에 클릭·입력·인증을 여러 개 묶지 않는다. 다음 사용자 동작도 필요하면 현재 동작 완료 후
  새 안내로 한 번에 하나씩 요청한다.
- `남은 단계`는 8개를 넘겨 나열하지 않는다. 더 많으면 총 개수와 가까운 8개까지만 보여준다.
- 사용자가 완료 신호를 알려주면 같은 질문을 반복하지 않고 identity·상태를 검증한 뒤 자동 흐름을
  즉시 재개한다.
...
먼저 `PLAN.md`의 준비 표를 읽는다. 동일한 도구·package·설치 scope가 이미 `INSTALL_APPROVED` 또는
`READY`이면 기존 동의를 재사용하고 다시 묻지 않는다. 새로 필요한 항목이나 system/project/host
scope가 넓어진 차이만 별도 목록으로 보여준다. 동의를 받으면 실행 전에 정확한 항목과 scope를
`INSTALL_APPROVED`로 기록해 `phase-connect.md`와 다음 세션이 같은 승인을 재사용하게 한다.
...
직접 한다. 그 단계가 나타나면 위 `USER_ACTION` 형식으로 필요한 한 동작만 요청하고, 완료되면
에이전트가 즉시 이어서 한다.
...
- 필요한 항목과 제외 항목을 실제로 확인했고, 미해결 설치·가입·계정·권한만 `PLAN.md` 표에 과장 없이
  기록돼 있다. 준비된 항목과 제외 항목 전체 목록은 복제하지 않는다.
...
필요하면 위 변경분 카드로 요청하고, 없으면 조용히 `phase-connect.md`에서 실제 준비를 실행한다.
