...
6. 2~5의 수정·재검증과 슬라이스 배포 중에는 `PLAN.md`를 건드리지 않는다. 테스트·CI·배포 로그와
   실제 URL이 중간 증거다. `PLAN.md`는 사람 결정, 실제 handoff, 최종 판정에서만 구현 위치·상태·
...
commit 전에는 이번 슬라이스의 코드·테스트·설정만 정확한 경로로 stage한다. 사람 결정이나 handoff로
실제로 바뀐 경우에만 `PLAN.md`를 포함하고, 사용자가 과정 기록을 요청해 기록 조건을 충족한 경우에만
`MEMORY.md`를 포함한다. `git diff --cached --name-only`를 확인하고 기존 사용자 변경을 함께 넣지 않는다.
...
첫 결과만을 기록하기 위해 `PLAN.md`를 고치거나 문서 전용 commit을 다시 만들지 않는다. 사용자에게
다음처럼 알리고 검증을 계속한다.
...
있어도 완료 조건이 중요한 변경은 production URL에서 다시 확인한다. 각 슬라이스에는 코드·테스트·
필요한 설정만 담고, 중간 체크 상태 문서를 덧붙이지 않는다.
...
모든 REQ의 구현이 끝났으면 중간 체크박스를 만들지 않고 `README.md`에 한 줄 소개, 실행·배포 방법,
필요한 환경변수 **이름만** 적는다. 실제 값은 적지 않는다.

사용자가 과정 기록을 요청했을 때만 `memory-log.md`에 따라 필요한 항목을 남긴다. 사용자가 교육용
사례 카드를 요청했을 때만 `case-card.md`를 따라 `CASE.md`를 만든다. 구현 내용만으로 두 파일을
자동 생성하지 않는다.
