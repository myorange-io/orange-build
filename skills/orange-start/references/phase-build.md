...
1. 관련 REQ의 정상·빈 입력·실패 시나리오를 먼저 적는다.
2. UI, 서버 처리, 데이터·외부 연동을 끝까지 구현한다.
3. 가능한 가장 작은 자동 테스트를 추가한다. 핵심 계산·변환은 단위 테스트, 사용자 흐름은 통합
   또는 브라우저 테스트로 둔다.
4. `package.json`의 실제 스크립트를 확인해 lint·test·production build를 실행한다.
...
5. 코드와 `PLAN.md`의 구현 위치·검증 결과·상태를 **같이** 갱신한다. PASS는 명령 성공뿐 아니라
   완료 조건의 기대 결과를 실제 관찰했을 때만 쓴다.
