...
`MEMORY.md`가 있고 실제 취약점 수정, 사용자가 수용한 잔여 위험, 다음 점검 기준이 달라진 결정이
있을 때만 '보안 점검' 항목을 **한 개 덧붙인다**(append, 덮어쓰지 않는다). 전부 OK이거나 이전과
같은 결과면 파일을 수정하지 않는다. 기록할 때는 점검한 항목 / 결과 / 수정한 것과 **어떻게
