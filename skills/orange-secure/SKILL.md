...
`MEMORY.md`가 있고 사용자가 과정 기록 유지를 요청했으며 실제 취약점 수정, 사용자가 수용한 잔여
위험, 다음 점검 기준이 달라진 결정이 있을 때만 '보안 점검' 항목을 **한 개 덧붙인다**(append,
덮어쓰지 않는다). 전부 OK이거나 이전과 같은 결과면 파일을 수정하지 않는다. 기록할 때는 점검한 항목 / 결과 / 수정한 것과 **어떻게
고쳤는지** / 남은 위험과 **그렇게 판단한 근거**를 구체적으로 적는다. 날짜는 `date +%Y-%m-%d`로
얻고 형식은 `../orange-start/references/memory-log.md`를 참고한다. 예:
